...
const generateCaptcha = (width = 450, height = 150, length = 5) => {
  ...
  const chars = 'QWERTYUIASDFGHJKZXCVBNMLOP7894561230qwertyuizxcvbnmlopasdfghjk';
  ...
  // Buat Decoy
            const decoyText = Array.from({length:length+Math.floor((width * height) / 10000)}, () => chars[Math.floor(Math.random() * chars.length)]).join('');
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    ctx.save();
    ctx.translate((width / (length + 1)) * (i + 1), height/2 + randomRange(-fontSize/2,fontSize/2));
    ctx.rotate(angle);
